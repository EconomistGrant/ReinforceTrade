...
class TradingEnv(py_environment.PyEnvironment):
    ...
    def action_spec(self):
        return BoundedArraySpec(shape = (), dtype = np.float32, minimum=0.0, maximum=3.0, name = 'action')
    ...
    def _execute_trade(self,action):
        ...
        if trade < 1:
            ...
            cash = prev_cash + trade * trade_price
            holding = prev_holding - trade
        
        elif trade > 2:
            # buy, limit cash >= 0
            cash = prev_cash - (trade - 2) * trade_price
            holding = prev_holding + (trade - 2)
